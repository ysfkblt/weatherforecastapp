...
const Journal = (props) => {
  ...
  useEffect(() => {
    ...
    async function getworms() {
      ...
      await setWormsBackup(
        datas.docs.map((doc) => ({ ...doc.data(), id: doc.id }))
      );
    }
    getworms();
  }, []);
  ...
  const uploadEntry = async () => {
    ...
    uploadBytes(imageRef, imageToUpload).then((snapshot) => {
      ...
      getDownloadURL(snapshot.ref).then(async (url) => {
        const newDoc = await addDoc(wormCollection, {
          image: url,
          notes: newNotes,
          date: date,
        });
        setWorms((prev) => [
          ...prev,
          { id: newDoc.id, image: url, notes: newNotes, date: date },
        ]);
        setWormsBackup((prev) => [
          ...prev,
          { id: newDoc.id, image: url, notes: newNotes, date: date },
    ...
    const editInputArea1 = document.querySelectorAll(".add-entry-area");
    const currentEditInputArea1 = editInputArea1[0];
      ...
      <div className="page-container">
        <div className="journal-container">
          ...
          <div className="journal-form">
            <h1 className="journal-form-heading" onClick={(evt) => addnotes()}>
              {" "}
              Add new entry
            </h1>
            <div className="add-entry-area">
              <div className="imgUpload">
                <input
                  className="journal-form-date-input"
                  type="date"
                  value={date}
                ...
                <div>
                  <input
                    className="journal-form-file-input button"
                    type="file"
                    onChange={(event) => {
                ...
                <div className="journal-notes-container">
                  ...
                  <div>
                    <button
                      className="journal-notes-image-upload-button"
                      onClick={uploadEntry}
                    >
                      {" "}
                      Upload Entry
          ...
          <div className="journal-previous-entries-container">
            <input
              ...
              size={50}
            />
            <p className="journal-previous-entries-heading">
              {" "}
              Previous entries:{" "}
            </p>
            <div>
                ...
                ? worms.map((entryData, index) => {
                    return (
                      <div className="journal-previous-entries" key={index}>
                        <h2 className="journal-previous-entries-date">
                          {" "}
                          {entryData.date}{" "}
                        </h2>
                        <img
                          src={entryData.image}
                          className="journal-previous-entries-image"
                        />
                        ...
                        <div>
                          <button
                            onClick={(event) => editNotes(entryData, index)}
                          ...
                          <span className="editArea">
                            ...
                            />
                            <button
                              onClick={(event) => submitEdit(entryData, index)}
                            >
                              Submit
                            </button>
                          </span>
                          <button onClick={(event) => deleteNotes(entryData)}>
                            Delete
                          </button>
                        </div>
                      </div>
